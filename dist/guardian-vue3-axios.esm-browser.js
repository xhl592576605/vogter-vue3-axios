/*!
  * @guardian/vue3-axios v0.0.1
  * (c) 2021 @guardian/vue-axios axios's extend
  * @license MIT
  */
...
class ComponentsApiService {
    ...
    $initAppWatch() {
        ...
        // watch out  filter
        app.$watch(() => _.cloneDeep(app.filter), (newVal, oldVal) => {
            this.$setApiParams();
            this.$setUiParams();
        }, { deep: true, immediate: false });
        // watch my filter
        app.$watch(() => _.cloneDeep(app.myFilter), (newVal, oldVal) => {
            this.$setApiParams();
            this.$setUiParams();
        }, { deep: true, immediate: false });
        // watch apiParms and fetchApiData
        app.$watch(() => _.cloneDeep(app.apiParams), (newVal, oldVal) => {
        ...
        app.$watch(() => _.cloneDeep(app.uiParams), (newVal, oldVal) => {
            if (_.isEqual(newVal, oldVal)) {
                return;
            }
            if (app.apiSuspend) {
        ...
        // watch api  ApiData
        app.$watch(() => _.cloneDeep(app.api), (newVal, oldVal) => {
            if (_.isEqual(newVal, oldVal)) {
                return;
            }
    ...
    $getApiOptions(api, filter) {
        ...
        axiosOptions = _.merge({}, axiosOptions, bodyParams
            ? {
                params: queryParams,
                data: bodyParams
            }
